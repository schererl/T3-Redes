...
def send(packet, topology):
    ...
    if packet.MAC_dst == ":FF": #mac_broadcast:
        ...
        for n in topology.nodes:
            if Utils.apply_mask(n.ip_prefix) == Utils.apply_mask(arp.IP_dst) and  n.ip_prefix != arp.IP_src: #envia pra todo mundo menos pra sí mesmo
...
def receive(node, packet, topology):
    if packet.type_ == "ARP":
        ...
        if arp.operation == 1: # request **send reply
        ...
        elif arp.operation == 2: # reply **update arp_table
            node.arp_table[arp.IP_src] = arp.MAC_src
